// experiment: fix compressed memory
...
void testCUCompress(int w, int d, int k, int sig_width = 1) {
    CUSketch cm[CM_NUM];
    bool NO_DP = sig_width > 1;
    ...
    if (!NO_DP)
        cm[4].compressDP(k, sig_width);
    cout << "CU sketch: k = " << k << ", sig_width = " << sig_width << ":"
         << endl;
    for (int i = 1; i < CM_NUM - NO_DP; i++) {
        double ARE = 0;
        double AAE = 0;
...
void testCountCompress(int w, int d, int k, int sig_width = 1) {
    CountSketch cm[CM_NUM];
    bool NO_DP = sig_width > 1;
    ...
    cm[1].compressSum(k, sig_width);
    cm[2].compressMedian(k, sig_width);
    cm[3].compressIterationByAverage(k, sig_width);
    if (!NO_DP)
        cm[4].compressDP(k, sig_width);
    cout << "Count sketch: k = " << k << ", sig_width = " << sig_width << ":"
         << endl;
    for (int i = 1; i < CM_NUM - NO_DP; i++) {
        double ARE = 0;
        double AAE = 0;
